...
test('测试用户注册，按正确的发送参数，测试结果应该注册成功', async () => {
...
test('测试用户注册，输入不正确的邮箱进行注册，测试结果应该注册失败', async () => {
  const res = await server.post('/api/v1/admin/register').send({
    nickname,
    email: `not a mail@m`,
    password1: password,
...
test('测试用户注册，输入两次密码不正确，测试结果注册失败', async () => {
  const res = await server.post('/api/v1/admin/register').send({
    nickname,
    email,
    password1: password,
...
test('测试登录，使用正确的邮箱和密码，测试结果应该登录成功', async () => {
...
test('测试登录，使用正确的邮箱和错误密码，测试结果应该登录失败', async () => {
  const res = await server.post('/api/v1/admin/login').send({
    email,
...
test('测试登录，使用不同的的邮箱和正确密码，测试结果应该登录失败', async () => {
  const res = await server.post('/api/v1/admin/login').send({
    email: '1024' + email,
    password: password
  })
